...
class PressureElement(DisplayElement):
    label = "pressure"
    ...
    def __init__(self, sense: SenseHat = None, *args, **kwargs):
        ...
        super(PressureElement, self).__init__(*args, **kwargs)
    ...
    @property
    def value(self):
        return f"{self._value / 1000:.3f}bar"
    ...
    def update(self, timestamp, event: InputEvent = None) -> bool:
        ...
        if self.last_update + 1 < timestamp:
            self.value = self.sense.get_pressure()
            self.last_update = timestamp
            self.update_buffer()
            self.should_redraw = True

        return super(PressureElement, self).update(timestamp, event)
